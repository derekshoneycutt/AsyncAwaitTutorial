...
/// The use of AsyncLocal is included in the Run method to demonstrate that thread local storage works when you
/// run the actions on the execution context captured for them. Otherwise, it is always just 0 when run on the
/// thread pool from the previous samples.
/// In a lot of Windows programming, thread local storage is important concerning the UI thread.
/// Monitoring and running actions on the execution context will cause them to run on the UI thread as is
/// appropriate as well. Later samples will expand on this core knowledge and how to apply it in async/await.
/// </para>
/// </summary>
