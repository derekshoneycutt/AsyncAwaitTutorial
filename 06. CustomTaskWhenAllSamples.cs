...
public class MyTaskMulti
{
    ...
    public static MyTaskMulti Run(Action action)
    {
        MyTaskMulti returnTask = new();
    ...
    public static MyTaskMulti WhenAll(params IEnumerable<MyTaskMulti> tasks)
    {
        MyTaskMulti returnTask = new();

        List<MyTaskMulti> useTasks = [.. tasks];
        if (useTasks.Count < 1)
        {
            returnTask.SetResult();
        }
        else
        {
            ...
            foreach (MyTaskMulti task in useTasks)
            {
...
public static class CustomTaskWhenAllSamples
{
    ...
    public static void Run(ParseResult parseResult)
    {
        ...
        List<MyTaskMulti> tasks = [];
        for (int i = 0; i < threadCount; ++i)
        {
            mod.Value = 10 * i;
            tasks.Add(MyTaskMulti.Run(() =>
                InstanceMethod(1 + mod.Value, 5 + mod.Value, 10001 + mod.Value, 10005 + mod.Value)));
        }

        MyTaskMulti.WhenAll(tasks).Wait();
    }
}
