...
The point of this project is to provide a series of 25 samples that progressively build to
...
5. Custom Task Completion : Demonstrates creating a basic task completion class to track tasks on the thread pool
...
11. State Machine : Demonstrates a basic C-style state machine for reference
...
14. Task iterator : Demonstrates iterating over Tasks returned by yield return to simulate async/await (Uses custom task class because TaskCompletionSource not introduced yet)
...
17. TaskCompletionSource : Demonstrates how to represent synchronous or managed operations alongside asynchronous code via a custom managed Thread and TaskCompletionSource
...
19. Enumerable of Tasks : Gets a basic feel for the logic behind IAsyncEnumerable
...
22. Custom Channels : Demonstrates a custom, basic channel class that shows the basic motivations and how it works
...
and understood Sample 18, with CancellationToken use. IAsyncEnumerable is also extremely
valuable in modern C#, and Channels help them, so I would personally recommend counting
all 25 absolutely essential. That said, many engineers stop at a loose understanding up to
about sample 16, missing details such as the ExecutionContext and ConfigureAwait along the way.
